...
export class UsersController {
  constructor(private readonly usersService: UsersService, private readonly jwt: JwtAuth) { }
  ...
  async getUserById(@Param('id') id: string, @Req() req: Request, @Res() res: Response) {
    try {
      ...
      console.error(error);
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ status: "error", message: "Failed to fetch user", statusCode: HttpStatus.INTERNAL_SERVER_ERROR })
    }
  }
}
